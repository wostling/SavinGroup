...
def plot_spectrum(data, filename, second, fit_model, baseline, leg, fig, ax):
    path = filename
    ...
    if second != "both":
        ...
        #else:
            #Spath = path.replace(".pdf", "_gauss.pdf")
...
def analyze_spectrum_thread(window, datafile, fiducial, fit_model, baseline, leg, commandline=False):
    ...
    if leg == "Leg 1":
        for i in range(num):
            ...
            fid_y = []
            for line in data:
                bpm_x.append(float(line[0]))
                bpm_y.append(float(line[1]))
                fid_x.append(float(line[2]))
                fid_y.append(float(line[3]))
            ...
            data = [bpm_x, bpm_y, fid_x, fid_y]
            # Get base path
            path = datafile.replace(".dat", "_plot.pdf")
            if fiducial:
                second = "fiducial"
            else:
                second = "none"
            plot_spectrum(data, path, second, fit_model, baseline, leg, fig, ax)
        if not commandline:
        ...
        for i in range(num):
            ...
            fid_y = []
            for line in data:
                bpm_x.append(float(line[4]))
                bpm_y.append(float(line[5]))
                fid_x.append(float(line[6]))
                fid_y.append(float(line[7]))
...
def analyze_spectrum(window, filename, fiducial, fitmodel, baseline, leg, commandline=False):
    threading.Thread(target=analyze_spectrum_thread, args=(window, filename, fiducial, fitmodel, baseline, leg), daemon=True).start()
...
def main(args):
    if not args.commandline:
        ...
        layout = [
            ...
            [sg.Frame(layout=[
                [sg.Text('Choose Leg to plot')],
                [sg.InputOptionMenu(('Leg 1', 'Leg 2'), key="-LEG-")],
                ], title='Leg plotter', relief=sg.RELIEF_SUNKEN, tooltip='Use these to set flags'),
            ],
            [sg.Frame(layout=[
        ...
        while True:
            ...
            if event == "-BOK-":
                ...
                window.refresh()
                analyze_spectrum(window, values["-FILENAME-"], values["-FIDUCIAL-"], values["-FITMODEL-"], values["-BASELINE-"], values["-LEG-"])
